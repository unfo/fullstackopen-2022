...
const MOCKUP_USER_CREDS = {
  username: 'mockup_user',
  passwordHash: '3732ae9d266a0f9a35b8ed26d08086fccb693dc0',
}; // used only in these tests
...
describe('4.14* blogilistan laajennus, step2', () => {
  ...
  test('trying to update non-existent id is 404', async () => {
    ...
    const emptyBlog = {};
    await api.put(`/api/blogs/${id}`).send(emptyBlog).expect(404);
    // const blogs = await helper.blogsInDb();
  ...
  test('trying to update malformed id is 400', async () => {
    const id = 'foobar';
    const emptyBlog = {};
    await api.put(`/api/blogs/${id}`).send(emptyBlog).expect(400);
  });
});
